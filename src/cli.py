...
def main():
    ...
    invoked_as = invoked_as.replace(".exe", "")

    args = sys.argv[1:]
    arg_string = " ".join(args).lower()

    # 1. Handle Bare Command (no args) — show help
    if not args and invoked_as in BARE_COMMANDS:
        show_help()
        return
    ...
    if args and args[0].lower() == "model-path":
        ...
        if set_model_path(path):
            ...
            print("❌ Setup failed. Check the path and try again.")
        print("─" * 55 + "\n")
        return
    ...
    if "context-logs off" in arg_string:
        context_logs_enabled = False
    if "context-logs on" in arg_string:
        context_logs_enabled = True
